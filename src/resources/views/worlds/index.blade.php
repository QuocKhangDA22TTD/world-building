...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">{{ __('My Worlds') }}</h1>
        <p class="text-gray-400">{{ __('Explore and manage your creative universes') }}</p>
    </div>
...
<div class="glass-card rounded-2xl p-12 text-center animate-fade-in-up">
    ...
    <h3 class="text-xl font-bold text-gray-800 mb-2">{{ __('No worlds yet') }}</h3>
    <p class="text-gray-500 mb-6 max-w-md mx-auto">{{ __('Start building your first world and bring your imagination to life!') }}</p>
                ...
                <tr class="animate-fade-in-up" style="opacity: 0; animation-delay: {{ $index * 0.05 }}s;">
                    <td class="px-6 py-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="font-semibold text-gray-800">{{ $world->name }}</p>
                                <p class="text-xs text-gray-400">{{ $world->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-gray-600 text-sm max-w-xs truncate">{{ $world->description ?: __('No description') }}</p>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center space-x-2">
                            <a href="{{ route('worlds.show', $world) }}" class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="{{ __('View') }}">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                            </a>
                            <a href="{{ route('worlds.edit', $world) }}" class="p-2 text-green-500 hover:bg-green-50 rounded-lg transition-colors" title="{{ __('Edit') }}">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
